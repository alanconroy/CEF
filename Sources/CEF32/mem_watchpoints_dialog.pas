...
type
  ...
  public // API...
      ...
      procedure Set_Data( Watchpoints : TCEF_Watchpoint_Manager ; Port : boolean ) ;
  end ; // TMemory_Watchpoints_Dialog

procedure Add_Breakpoint( Component : TComponent ;
    Watchpoints : TCEF_Watchpoint_Manager ; Base : int64 ; Size : longint ;
    Port : boolean ) ;

procedure Add_Watchpoint( Component : TComponent ; Component_Type : integer ;
    Watchpoints : TCEF_Watchpoint_Manager ; Base : int64 ; Size : longint ;
    Memory : boolean ; var Address : int64 ; var Access : integer ) ;

procedure Show_Watchpoints( CPU : TComponent ; Watchpoints : TCEF_Watchpoint_Manager ;
    Ports : boolean ) ;
...
var Memory_Watchpoints_Dialog : TMemory_Watchpoints_Dialog ;
...
begin
	...
	for Loop := 0 to Watchpoints.Count - 1 do
        ...
        Watchpoints.Create_Watchpoint( Address, New_Watchpoint.Size.Value, Access ) ;
...
procedure Add_Breakpoint( Component : TComponent ;
    Watchpoints : TCEF_Watchpoint_Manager ; Base : int64 ;
    Size : longint ; Port : boolean ) ;
...
begin
    ...
    Memory_Watchpoints_Dialog.Set_Data( Watchpoints, Ports ) ;
...
begin
    ...
    begin
        if( _Watchpoints.Has_Sizes ) then
        begin
            Grid.Cells[ 0, Loop + 1 ] := inttostr( _Watchpoints.Watchpoint_At_Index( Loop ).Address ) ;
            Grid.Cells[ 1, Loop + 1 ] := inttostr( _Watchpoints.Watchpoint_At_Index( Loop ).Size ) ;
        end else
        begin
            ...
            begin
                Grid.Cells[ 0, Loop + 1 ] := CPU.CPU.Register_Name( _Watchpoints.Watchpoint_At_Index( Loop ).Address ) ;
            end ;
            Grid.Cells[ 1, Loop + 1 ] := '' ;
        end ;
        S := '' ;
        if( _Watchpoints.Has_Accesses ) then
        begin
            if( ( _Watchpoints.Watchpoint_At_Index( Loop ).Access and Access_Read ) <> 0 ) then
            begin
                S := 'read' ;
            end ;
            if( ( _Watchpoints.Watchpoint_At_Index( Loop ).Access and Access_Write ) <> 0 ) then
            begin
                if( length( S ) > 0 ) then
                begin
                    S := S + ', ' ;
            ...
            if( ( _Watchpoints.Watchpoint_At_Index( Loop ).Access and Access_Execute ) <> 0 ) then
            begin
                if( length( S ) > 0 ) then
                begin
                    S := S + ', ' ;
...
begin
    Grid.ColWidths[ 0 ] := ClientWidth div 3 - 2 ;
    Grid.ColWidths[ 1 ] := ClientWidth div 3 - 2 ;
    Grid.ColWidths[ 2 ] := ClientWidth div 3 - 2 ;
