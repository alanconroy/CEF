{
        Program Name : SOL20KB_Main
        Package Name : CEF
        Purpose      : Processor Technology SOL-20 keyboard component main form
	...
	Released to the public domain.
        ...
        *************************************************************
        ...
        This is the main form for the SOL-20 keyboard component for CEF.
...
type
  TMain_Form = class(TForm)
    ...
    Uppercase_Button: TSpeedButton;
    Local_Button: TSpeedButton;
    ...
    KP_Dash_Button: TSpeedButton;
    KP_Asterisk_Button: TSpeedButton;
    Button_Divide: TSpeedButton;
    ...
    KP_Dot_Button: TSpeedButton;
    KP_Plus_Button: TSpeedButton;
    ...
    Home_Cursor_Button: TSpeedButton;
    Right_Shift_Button: TSpeedButton;
    Mode_Select_Button: TSpeedButton;
    Local_LED: TShape;
    Uppercase_LED: TShape;
...
begin
    ...
    begin
        _SOL20KB._Keyboard.Add_Key( 'UP', 130, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'UP', 130, 0 ) ;
        exit ;
    end else
    if( Button = Down_Button ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'DOWN', 131, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'DOWN', 131, 0 ) ;
        exit ;
    end else
    if( Button = Left_Button ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'LEFT', 128, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'LEFT', 128, 0 ) ;
        exit ;
    end else
    if( Button = Right_Button ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'RIGHT', 129, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'RIGHT', 129, 0 ) ;
        exit ;
    end else
    if( Button = Left_Shift_Button ) then
    begin
        Shift_Lock_LED.Brush.Color := clGreen ;
        _SOL20KB._Keyboard.Add_Key( 'LEFT_SHIFT', 132, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'SHIFT LOCK', 134, 0 ) ;
        exit ;
    end else
    if( Button = Right_Shift_Button ) then
    begin
        Shift_Lock_LED.Brush.Color := clGreen ;
        _SOL20KB._Keyboard.Add_Key( 'LEFT_SHIFT', 133, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'SHIFT LOCK', 134, 0 ) ;
        exit ;
    end else
    if( Button = Left_Control_Button ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'LEFT_CONTROL', 142, 1 ) ;
        exit ;
    end else
    if( Button = Right_Control_Button ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'RIGHT_CONTROL', 143, 1 ) ;
        exit ;
    end else
    if( Button = Button_Divide ) then
    begin
        C := #$2F ;
    end else
    if( Button.Caption = 'BREAK' ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'BREAK', 0, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'BREAK', 0, 0 ) ;
        exit ;
    end else
    ...
    begin
        if( Shift_Lock_LED.Brush.Color = clGreen ) then
        begin
            Shift_Lock_LED.Brush.Color := clLime ;
            _SOL20KB._Keyboard.Add_Key( 'SHIFT LOCK', 134, 1 ) ;
        end else
        begin
            Shift_Lock_LED.Brush.Color := clGreen ;
            _SOL20KB._Keyboard.Add_Key( 'SHIFT LOCK', 134, 0 ) ;
    ...
    if( Button.Caption = 'UPPER'#13'CASE' ) then
    begin
        if( Uppercase_LED.Brush.Color = clGreen ) then
        begin
            Uppercase_LED.Brush.Color := clLime ;
            _SOL20KB._Keyboard.Add_Key( 'UPPER CASE', 136, 1 ) ;
        end else
        begin
            Uppercase_LED.Brush.Color := clGreen ;
            _SOL20KB._Keyboard.Add_Key( 'UPPER CASE', 136, 0 ) ;
        end ;
        exit ;
    end else
    if( Button.Caption = 'LOCAL' ) then
    begin
        if( Local_LED.Brush.Color = clGreen ) then
        begin
            Local_LED.Brush.Color := clLime ;
            _SOL20KB._Keyboard.Add_Key( 'LOCAL', 137, 1 ) ;
        end else
        begin
            Local_LED.Brush.Color := clGreen ;
            _SOL20KB._Keyboard.Add_Key( 'LOCAL', 137, 0 ) ;
        end ;
        exit ;
    end else
    if( Button.Caption = 'CLEAR' ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'CLEAR', 138, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'CLEAR', 138, 0 ) ;
        exit ;
    end else
    if( Button.Caption = 'HOME'#13'CURSOR' ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'HOME CURSOR', 139, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'HOME CURSOR', 139, 0 ) ;
        exit ;
    end else
    if( Button.Caption = 'LOAD' ) then
    begin
        C := #$8C ;
    end else
    if( Button.Caption = 'MODE'#13'SELECT' ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'MODE SELECT', 141, 1 ) ;
        _SOL20KB._Keyboard.Add_Key( 'MODE SELECT', 141, 0 ) ;
        exit ;
    end else
    ...
    _SOL20KB._Keyboard.Add_Key( C, ord( C ), 1 ) ;
    _SOL20KB._Keyboard.Add_Key( C, ord( C ), 0 ) ;

    if( Right_Shift_Button.Down ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'SHIFT', 132, 0 ) ;
    end ;
    Right_Shift_Button.Down := False ;
    Left_Shift_Button.Down := False ;
    if( Left_Control_Button.Down ) then
    begin
        _SOL20KB._Keyboard.Add_Key( 'CONTROL', 133, 0 ) ;
    end ;
...
begin
    ...
    Button_Underscore.Caption := '_'#13'DEL' ;
    Button_Less_Than.Caption := '<'#13',' ;
    Button_Greater_Than.Caption := '>'#13'.' ;
    Button_Question_Mark.Caption := '?'#13'/' ;
    Shift_Lock_Button.Caption := 'SHIFT'#13'LOCK' ;
    Uppercase_Button.Caption := 'UPPER'#13'CASE' ;
    Line_Feed_Button.Caption := 'LINE'#13'FEED' ;
    Mode_Select_Button.Caption := 'MODE'#13'SELECT' ;
    Home_Cursor_Button.Caption := 'HOME'#13'CURSOR' ;
    ...
    Uppercase_Button := Update_Key( Uppercase_Button, clWhite ) ;
    ...
    Button_Left_Brace := Update_Key( Button_Left_Brace, clDkGray ) ;
    Button_Vertical := Update_Key( Button_Vertical, clDkGray ) ;
    Button_Right_Brace := Update_Key( Button_Right_Brace, clDkGray ) ;
    Button_Divide := Update_Key( Button_Divide, clBlack ) ;
    Line_Feed_Button := Update_Key( Line_Feed_Button, clBlack ) ;
    Button_Accent := Update_Key( Button_Accent, clDkGray ) ;
    Shift_Lock_Button := Update_Key( Shift_Lock_Button, clDkGray ) ;
    ...
    Button_Asterisk := Update_Key( Button_Asterisk, clDkGray ) ;
    Button_Underscore := Update_Key( Button_Underscore, clDkGray ) ;
    ...
    Home_Cursor_Button := Update_Key( Home_Cursor_Button, clBlack ) ;
    Right_Shift_Button := Update_Key( Right_Shift_Button, clDkGray ) ;
    Mode_Select_Button := Update_Key( Mode_Select_Button, clWhite ) ;
    Local_Button := Update_Key( Local_Button, clWhite ) ;
    Load_Button := Update_Key( Load_Button, clWhite ) ;
    Break_Button := Update_Key( Break_Button, clWhite ) ;
    Clear_Button := Update_Key( Clear_Button, clBlack ) ;
    KP_Dash_Button := Update_Key( KP_Dash_Button, clBlack ) ;
    KP_Asterisk_Button := Update_Key( KP_Asterisk_Button, clBlack ) ;
    KP_Dot_Button := Update_Key( KP_Dot_Button, clBlack ) ;
    KP_Plus_Button := Update_Key( KP_Plus_Button, clBlack ) ;
    Repeat_Button := Update_Key( Repeat_Button, clBlack ) ;
    Return_Button := Update_Key( Return_Button, clBlack ) ;

    TKey_Button( Local_Button ).LED := Local_LED ;
    TKey_Button( Local_Button ).LED_Position := Point( Local_LED.Left, Local_LED.Top ) ;
    TKey_Button( Uppercase_Button ).LED := Uppercase_LED ;
    TKey_Button( Uppercase_Button ).LED_Position := Point( Uppercase_LED.Left, Uppercase_LED.Top ) ;
    ...
    repeat
        ...
        begin
            ...
            begin
                Update_Key( TSpeedButton( Panel3.Controls[ Loop ] ), clDkGray ) ;
                Changes := True ; // Need to run through the loop again
            end ;
        end ;
    until ( not Changes ) ;

    Local_LED.BringToFront ;
    Uppercase_LED.BringToFront ;
