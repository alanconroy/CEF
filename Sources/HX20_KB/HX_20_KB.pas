{$N+}
{
	  ...
	  This unit Epson HX20 keyboard component for CEF.
...
uses // Borland...
     ...
     // HX20_KB...
     HX20KB_Main ; // TMain_Form

const HX20KB_Facility = -1 ;
      HX20KB_Success = 0 ;
      HX20KB_Component_Not_Found = 1 ;
      HX20KB_Invalid_Component = 2 ;
      HX20KB_No_Matching_Watchpoint = 3 ;

type THX20KB_UI = class ;
     THX20KB_KB = class ;

     THX20KB = class( TBase_Component )
                   ...
                   public { Public instance data... }
                       ...
                       _User_Interface : THX20KB_UI ;
                       _Keyboard : THX20KB_KB ;
     ...
     THX20KB_UI = class( TBase_User_Interface )
	...
	private // Instance data...
     ...
     THX20KB_KB = class( TBase_Keyboard )
                      private // Instance data...
                          Parent : THX20KB ;
                          Temp_Key : string ;
...
{ THX20KB_Debugger methods... }

type THX20KB_Debugger = class( TText_Debugger )
                                private
                                    _Memory : THX20KB ;
                                ...
                                public { API... }
                                    ...
                                    property Memory : THX20KB
                                                 read _Memory
                                                 write _Memory ;
                            end ;

function THX20KB_Debugger.Child( Ordinal : longint ) : PDebug_Interface ;
...
function THX20KB_Debugger.Count : longint ;
...
function THX20KB_UI.Translate_Error( Code : longint ) : string ;

var _Error : string ;

begin
    case Code of
        HX20KB_Success: _Error := 'Success' ;
        HX20KB_Component_Not_Found : _Error := 'Component not found' ;
        HX20KB_Invalid_Component : _Error := 'Invalid component' ;
        HX20KB_No_Matching_Watchpoint : _Error := 'No Matching Watchpoint' ;
...
function THX20KB_UI.Get_Hidden : boolean ;
...
procedure THX20KB_UI.Set_Hidden( Value : boolean ) ;
...
function THX20KB_UI.Get_Parent_Window : THandle ;
...
procedure THX20KB_UI.Set_Parent_Window( Value : THandle ) ;
...
function THX20KB_UI.Optimal_Height : integer ;
...
function THX20KB_UI.Optimal_Width : integer ;
...
function THX20KB_KB.Get_Key : PChar ;
...
function THX20KB_KB.Get_Key_Down( Name : PChar ) : boolean ;
...
function THX20KB_KB.Get_Key_Name( Index : integer ) : PChar ;
...
begin
    ...
    case Index of
        ...
        132 : Temp_Key_Name := 'DEL' ;
        133 : Temp_Key_Name := 'CONTROL' ;
        134 : Temp_Key_Name := 'CAPS LOCK' ;
        135 : Temp_Key_Name := 'LEFT_SHIFT' ;
        136 : Temp_Key_Name := 'RIGHT_SHIFT' ;
        137 : Temp_Key_Name := 'GRPH' ;
        138 : Temp_Key_Name := 'PAUSE' ;
        139 : Temp_Key_Name := 'MENU' ;
        140 : Temp_Key_Name := 'NUM' ;
        141 : Temp_Key_Name := 'HOME' ;
        142 : Temp_Key_Name := 'CLEAR' ;
        143 : Temp_Key_Name := 'INS' ;
        144 : Temp_Key_Name := 'SCREEN_DOWN' ;
        145 : Temp_Key_Name := 'SCREEN_UP' ;
        146 : Temp_Key_Name := 'PF1' ;
        147 : Temp_Key_Name := 'PF2' ;
        148 : Temp_Key_Name := 'PF3' ;
        149 : Temp_Key_Name := 'PF4' ;
        150 : Temp_Key_Name := 'PF5' ;
        151 : Temp_Key_Name := 'HOME_CLR' ;
        152 : Temp_Key_Name := 'SCRN' ;
        153 : Temp_Key_Name := 'INS_DEL' ;
        154 : Temp_Key_Name := 'UP_LEFT' ;
        155 : Temp_Key_Name := 'DOWN_RIGHT' ;
    end ;
...
procedure THX20KB_KB.Add_Key( Key : string ; Value, Up : integer ) ;
...
function THX20KB.Default_Input : TComponent ;
...
function THX20KB.Default_Output : TComponent ;
...
function THX20KB.User_Interface : TUser_Interface ;
...
function THX20KB.Keyboard : TKeyboard ;
...
function THX20KB.Facility_Code : longint ;

begin
    Facility_Code := HX20KB_Facility ;
end ;


function THX20KB.Initialize( UI : TUI_Interface ) : TUnified_Exception ;
...
begin
    ...
    begin
        ...
        _User_Interface := THX20KB_UI.Create ;
        _User_Interface.Parent := self ;
        _User_Interface.Main_Form._UI := UI ;
        _User_Interface.Main_Form._HX20KB := self ;
        _Keyboard := THX20KB_KB.Create ;
        _Keyboard.Parent := self ;
    end ;
    Initialize := _User_Interface.Set_Error( HX20KB_Success ) ;
end ; { THX20KB.Initialize }


function THX20KB.Terminate : TUnified_Exception ;
...
begin
    ...
    Outputs.Free ;
    Outputs := nil ;
    Terminate := _User_Interface.Set_Error( HX20KB_Success ) ;
    _User_Interface.Free ;
    _User_Interface := nil ;
    _Keyboard.Free ;
    _Keyboard := nil ;
    Free ;
end ; { THX20KB.Terminate }



function THX20KB.Clear_Watchpoint( Address : int64 ; Memory : boolean ;
...
function THX20KB.Component_Type : longint ;
...
function THX20KB.Connect_Input( Component : TComponent ) : TUnified_Exception ;

begin
    if( Component = nil ) then
    begin
        Connect_Input := _User_Interface.Set_Error( HX20KB_Invalid_Component ) ;
        exit ;
    end ;
    Inputs.Add( Component ) ;
    Connect_Input := _User_Interface.Set_Error( HX20KB_Success ) ;
end ;


function THX20KB.Connect_Output( Component : TComponent ) : TUnified_Exception ;

begin
    if( Component = nil ) then
    begin
        Connect_Output := _User_Interface.Set_Error( HX20KB_Invalid_Component ) ;
        exit ;
    end ;
    Outputs.Add( Component ) ;
    Connect_Output := _User_Interface.Set_Error( HX20KB_Success ) ;
end ;


function THX20KB.Debugger : TDebug_Interface ;

begin
    Result := THX20KB_Debugger.Create ;
    THX20KB_Debugger( Result ).Memory := Self ;
end ;


function THX20KB.Deposit( Address : int64 ; Size : longint ;
    Buffer : pointer ; Memory : boolean ) : TUnified_Exception ;
...
function THX20KB.Disconnect_Input( Component : TComponent ) : TUnified_Exception ;

begin
    if( ( Component = nil ) or ( Inputs.IndexOf( Component ) = -1 ) ) then
    begin
	Disconnect_Input := _User_Interface.Set_Error( HX20KB_Component_Not_Found ) ;
    end else
    begin
	Disconnect_Input := _User_Interface.Set_Error( HX20KB_Success ) ;
	Inputs.Remove( Component ) ;
    end ;
end ;


function THX20KB.Disconnect_Output( Component : TComponent ) : TUnified_Exception ;

begin
    if( ( Component = nil ) or ( Outputs.IndexOf( Component ) = -1 ) ) then
    begin
	Disconnect_Output := _User_Interface.Set_Error( HX20KB_Component_Not_Found ) ;
    end else
    begin
	Disconnect_Output := _User_Interface.Set_Error( HX20KB_Success ) ;
	Outputs.Remove( Component ) ;
    end ;
end ;


function THX20KB.Examine( Address : int64 ; var Size : longint ;
    Buffer : pointer ; Memory : boolean ) : TUnified_Exception ;
...
function THX20KB.Get_Access_Mode( Address : int64 ;
...
function THX20KB.Get_Profiling : boolean ;
...
function THX20KB.Get_Read_Latency : longint ;
...
function THX20KB.Get_Write_Latency : longint ;
...
function THX20KB.Input_Component( Index : longint ) : TComponent ;
...
const _Name : PChar = 'HX20 Keyboard'#0 ;

function THX20KB.Name : PChar ;
...
function THX20KB.Output_Component( Index : longint ) : TComponent ;
...
function THX20KB.Read( Address : int64 ; Size : longint ;
...
function THX20KB.Restore_Contents( Stream : TCOM_Stream ) : TUnified_Exception ;
...
function THX20KB.Restore_State( Stream : TCOM_Stream ) : TUnified_Exception ;
...
function THX20KB.Save_Contents( Stream : TCOM_Stream ): TUnified_Exception ;
...
function THX20KB.Save_State( Stream : TCOM_Stream ) : TUnified_Exception ;
...
function THX20KB.Set_Access_Mode( Low, High : int64 ; Memory : boolean ;
...
procedure THX20KB.Set_Profiling( _On, Children : boolean ) ;
...
procedure THX20KB.Set_Read_Latency( Value : longint ) ;
...
function THX20KB.Set_Watchpoint( Address : int64 ; Memory : boolean ;
            Access : longint ) : TUnified_Exception ;
...
procedure THX20KB.Set_Write_Latency( Value : longint ) ;
...
function THX20KB.Write( Address : int64 ; Value, Size : longint ;
...
procedure THX20KB.Set_Tag( Value : longint ) ;
...
function THX20KB.Get_Tag : longint ;
...
function THX20KB.Get_Parent : TComponent ;
...
procedure THX20KB.Set_Parent( Component : TComponent ) ;
...
procedure THX20KB.Set_Up( P : PChar ) ;
