{$N+}
{
        ...
        *************************************************************
	    ...
	    This unit implements a generic memory component for CEF.  The emulated
        memory is allocated/reallocated in a single block (only grows) for
        speed.

	    The memory is infinitely fast (as far as the Emulator is concerned),
        single-port, static RAM, with a maximum address size of 31 bits.
...
type TCEF_SRAM_Memory = class ;

     TCEF_SRAM = class( TComponent )
                           private // Instance data...
                               _Memory : TCEF_SRAM_Memory ;
                               _Tag : longint ;
                               _Parent : TComponent ;
     ...
     TCEF_SRAM_Memory = class( TMemory )
                                  ...
                                  private // Internal utility routines...
                                      ...
                                      procedure Check_Size( Address : int64 ) ;

                                  public // API...
                                      function Set_Address_Range( Low, High : int64 ) : TUEC ;
                                          override ;
...
constructor TCEF_SRAM_Memory.Create ;
...
destructor TCEF_SRAM_Memory.Destroy ;
...
procedure TCEF_SRAM_Memory.Clear_Memory ;
...
function TCEF_SRAM.Default_Input : TComponent ;
...
function TCEF_SRAM.Default_Output : TComponent ;
...
procedure TCEF_SRAM_Memory.Load_File( const S : string ) ;
...
function TCEF_SRAM_Memory.Set_Error( Code : integer ) : TUEC ;
...
function TCEF_SRAM_Memory.Watchpoint_At( Address : int64 ) : TCEF_Watchpoint ;
...
procedure TCEF_SRAM_Memory.Check_Size( Address : int64 ) ;

begin
    if( Address > Used ) then
    begin
        Reallocmem( pointer( Data ), Used, Address ) ;
        Used := Address ;
    end ;
end ;
...
function TCEF_SRAM.Memory : TMemory ;
...
function TCEF_SRAM.Facility_Code : longint ;
...
function TCEF_SRAM.Initialize( UI : TUI_Interface ) : TUEC ;
...
begin
    ...
    _Memory := TCEF_SRAM_Memory.Create ;
    _Memory._UI := UI ;
    _Memory.Parent := self ;
...
function TCEF_SRAM.Terminate : TUEC ;
...
function TCEF_SRAM.Clear_Watchpoint( Address : int64 ; Memory : boolean ;
            Access : longint ) : TUEC ;
...
function TCEF_SRAM.Component_Type : longint ;
...
function TCEF_SRAM.Connect_Input( Component : TComponent ) : TUEC ;
...
function TCEF_SRAM.Connect_Output( Component : TComponent ) : TUEC ;
...
function TCEF_SRAM.Debugger : TDebug_Interface ;
...
function TCEF_SRAM.Deposit( Address : int64 ; Size : longint ;
    Buffer : pointer ; Memory : boolean ) : TUEC ;
...
begin
    ...
    _Memory.Check_Size( Address + Count ) ;
    move( _Buffer[ 0 ], _Memory.Data[ Address ], Count ) ;
    Deposit := _Memory.Set_Error( CEFMemoryErr_Success ) ;
end ; { TCEF_Memory.Deposit }


function TCEF_SRAM.Disconnect_Input( Component : TComponent ) : TUEC ;
...
function TCEF_SRAM.Disconnect_Output( Component : TComponent ) : TUEC ;
...
function TCEF_SRAM.Examine( Address : int64 ; var Size : longint ;
    Buffer : pointer ; Memory : boolean ) : TUEC ;
...
begin
    ...
    _Memory.Check_Size( Address + Count ) ;
    move( _Memory.Data[ Address ], _Buffer[ 0 ], Count ) ;
    Examine := _Memory.Set_Error( CEFMemoryErr_Success ) ;
end ; { TCEF_Memory.Examine }


function TCEF_SRAM.Get_Access_Mode( Address : int64 ;
...
function TCEF_SRAM.Get_Profiling : boolean ;
...
function TCEF_SRAM.Get_Read_Latency : longint ;
...
function TCEF_SRAM.Get_Write_Latency : longint ;
...
function TCEF_SRAM.Input_Component( Index : longint ) : TComponent ;
...
function TCEF_SRAM.Output_Component( Index : longint ) : TComponent ;
...
function TCEF_SRAM.Read( Address : int64 ; Size : longint ;
...
begin
    if( ( Address >= _Memory._Low ) and ( IO_Type = IO_Type_Memory ) ) then
    begin
...
function TCEF_SRAM.Restore_Contents( Stream : TCOM_Stream ) : TUEC ;
...
function TCEF_SRAM.Restore_State( Stream : TCOM_Stream ) : TUEC ;
...
function TCEF_SRAM.Save_Contents( Stream : TCOM_Stream ): TUEC ;
...
end ; { TCEF_SRAM.Save_Contents }


function TCEF_SRAM.Save_State( Stream : TCOM_Stream ) : TUEC ;
...
function TCEF_SRAM.Set_Access_Mode( Low, High : int64 ; Memory : boolean ;
    Typ : longint ) : TUEC ;
...
procedure TCEF_SRAM.Set_Profiling( _On, Children : boolean ) ;
...
procedure TCEF_SRAM.Set_Read_Latency( Value : longint ) ;
...
function TCEF_SRAM.Set_Watchpoint( Address : int64 ; Memory : boolean ;
            Access : longint ) : TUEC ;
...
procedure TCEF_SRAM.Set_Write_Latency( Value : longint ) ;
...
procedure TCEF_SRAM.Show_Status ;
...
function TCEF_SRAM.Write( Address : int64 ; Value, Size : longint ;
...
function TCEF_SRAM_Memory.Set_Address_Range( Low, High : int64 ) : TUEC ;
...
procedure TCEF_SRAM_Memory.Get_Address_Range( var Low, High : int64 ) ;
...
procedure TCEF_SRAM.Set_Tag( Value : longint ) ;
...
function TCEF_SRAM.Get_Tag : longint ;
...
function TCEF_SRAM.Get_Parent : TComponent ;
...
procedure TCEF_SRAM.Set_Parent( Component : TComponent ) ;
...
procedure TCEF_SRAM.Set_Up( P : PChar ) ;
...
begin
    ...
    Parser.Free ;
end ; // TCEF_SRAM.Set_Up


function TCEF_SRAM.Get_Logger : TCEF_Logger ;
...
procedure TCEF_SRAM.Set_Logger( Value : TCEF_Logger ) ;
...
procedure TCEF_SRAM_Memory.Dump( Start, Size : int64 ; Buffer : pointer ) ;

var P : Pchar ;

begin
    Check_Size( Start + Size ) ;
    P := PChar( Buffer ) ;
    move( Data[ Start ], P[ 0 ], Size ) ;
end ;


procedure TCEF_SRAM_Memory.Load( Start, Size : int64 ; Buffer : pointer ) ;
...
begin
    Check_Size( Start + Size ) ;
    P := PChar( Buffer ) ;
    move( P[ 0 ], Data[ Start ], Size ) ;
end ;


function TCEF_SRAM_Memory.Map_Virtual_To_Physical( Virt : int64 ) : int64 ;
