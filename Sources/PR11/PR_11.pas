{$N+}
{
        Program Name : PR11
        Package Name : CEF
        Purpose      : DEC PR11 component for CEF
        Institution  : 
	  ...
	  This unit implements a DEC PR11 UNIBUS paper tape reader.
...
const CEFPR11_Facility = -1 ;
      CEFPR11Err_Success = 0 ;
      CEFPR11Err_Invalid_Range = 1 ;
      CEFPR11Err_Component_Not_Found = 2 ;
      CEFPR11Err_No_Matching_Watchpoint = 3 ;
      CEFPR11Err_Access_Violation = 4 ;
      CEFPR11Err_Address_Out_Of_Range = 5 ;
      CEFPR11Err_Invalid_Component = 6 ;

type TPR11_IO = class;

     TPR11 = class( TBase_Component )
...
{ TCEF_PR11_Debugger methods... }

type TCEF_PR11_Debugger = class( TText_Debugger )
                                private
                                    _IO : TPR11 ;
...
function TCEF_PR11_Debugger.Child( Ordinal : longint ) : PDebug_Interface ;
...
function TCEF_PR11_Debugger.Count : longint ;
...
function tPR11.Translate_Error( Code : longint ) : string ;

var _Error : string ;

begin
    case Code of
        CEFPR11Err_Success: _Error := 'Success' ;
        CEFPR11Err_Invalid_Range: _Error := 'Invalid range' ;
        CEFPR11Err_Component_Not_Found: _Error := 'Component not found' ;
        CEFPR11Err_No_Matching_Watchpoint: _Error := 'No matching watchpoint' ;
        CEFPR11Err_Access_Violation: _Error := 'Access violation' ;
        CEFPR11Err_Address_Out_Of_Range: _Error := 'Address out of range' ;
        CEFPR11Err_Invalid_Component: _Error := 'Invalid component' ;
	    else _Error := 'Unknown error number ' + Num1( Code ) ;
    end ;
    Translate_Error := _Error ;
end ; { tPR11.Translate_Error }


function TPR11.Default_Input : TComponent ;
...
function TPR11.Default_Output : TComponent ;
...
function TPR11.Watchpoint_At( Address : int64 ) : TCEF_Watchpoint ;
...
function TPR11.Facility_Code : longint ;

begin
    Facility_Code := CEFPR11_Facility ;
end ;


function TPR11.Initialize( UI : TUI_Interface ) : TUnified_Exception ;
...
begin
    ...
    Initialize := Set_Error( CEFPR11Err_Success ) ;
    Init ;
end ; { TPR11.Initialize }


function TPR11.Terminate : TUnified_Exception ;
...
begin
    ...
    IO.Free ;
    IO := nil ;
    Terminate := Set_Error( CEFPR11Err_Success ) ;
end ; { TPR11.Terminate }



function TPR11.Clear_Watchpoint( Address : int64 ; Memory : boolean ;
            Access : longint ) : TUnified_Exception ;

begin
    Last_Error := Set_Error( CEFPR11Err_Success ) ;
    Clear_Watchpoint := Last_Error ;
    if( not Memory ) then
    begin
        Result := Watchpoint_List.Clear_Watchpoint( Address, Access ) ;
//        Clear_Watchpoint := Set_Error( CEFPR11Err_No_Matching_Watchpoint ) ;
    end ;
end ;


function TPR11.Component_Type : longint ;
...
function TPR11.Connect_Input( Component : TComponent ) : TUnified_Exception ;
...
begin
    ...
    begin
        Connect_Input := Set_Error( CEFPR11Err_Invalid_Component ) ;
        exit ;
    end ;
    ...
    Connect_Input := Set_Error( CEFPR11Err_Success ) ;
end ;


function TPR11.Connect_Output( Component : TComponent ) : TUnified_Exception ;
...
begin
    if( Component = nil ) then
    begin
        Connect_Output := Set_Error( CEFPR11Err_Invalid_Component ) ;
        exit ;
    end ;
    ...
    Connect_Output := Set_Error( CEFPR11Err_Success ) ;
end ;


function TPR11.Debugger : TDebug_Interface ;

begin
    Result := TCEF_PR11_Debugger.Create ;
    TCEF_PR11_Debugger( Result ).IO := Self ;
end ;


function TPR11.Deposit( Address : int64 ; Size : longint ; Buffer : pointer ;
...
begin
    Set_Error( 0 ) ;
    Result := nil ;
    if( ( not Memory ) or ( Octal( '777550' )> Address ) or ( Octal( '777553' ) < Address ) or ( Size = 0 ) ) then
    begin
        exit ;
    end ;
    ...
    try
        case Address of
            ...
            1 : // PRS high byte
                begin
                    Value := ( Value shl 8 ) or ( PRS and $FF ) ;
                    PRS := Value ;
                end ;
            2 : // PRB
                begin
                    if( Count = 1 ) then
                    begin
                        Value := Value or ( PRB and $FF00 ) ;
                    end ;
                    PRB := Value ;
                end ;
            3 : // PRB high byte
                begin
                    Value := ( Value shl 8 ) or ( PRB and $FF ) ;
                    PRB := Value ;
                end ;
        end ; // case Address of
    except
    end ;
    Deposit := Set_Error( CEFPR11Err_Success ) ;
end ; { TPR11.Deposit }


function TPR11.Disconnect_Input( Component : TComponent ) : TUnified_Exception ;
...
begin
    ...
    begin
        Disconnect_Input := Set_Error( CEFPR11Err_Component_Not_Found ) ;
    end else
    begin
	Disconnect_Input := Set_Error( CEFPR11Err_Success ) ;
        if( Parent <> nil ) then
        begin
...
function TPR11.Disconnect_Output( Component : TComponent ) : TUnified_Exception ;
...
begin
    ...
    begin
        Disconnect_Output := Set_Error( CEFPR11Err_Component_Not_Found ) ;
    end else
    begin
	Disconnect_Output := Set_Error( CEFPR11Err_Success ) ;
        if( Parent <> nil ) then
        begin
            Code := Child_Notice_Disconnect ;
            P := integer( Component ) ;
...
function TPR11.Examine( Address : int64 ; var Size : longint ;
...
begin
    Set_Error( 0 ) ;
    Result := nil ;
    if( ( not Memory ) or ( Octal( '777550' ) > Address ) or ( Octal( '777553' ) < Address ) ) then
    begin
        Examine := Set_Error( CEFPR11Err_Address_Out_Of_Range ) ;
        exit ;
    end ;
    ...
    Examine := Set_Error( CEFPR11Err_Success ) ;
end ; { TPR11.Examine }


function TPR11.Get_Access_Mode( Address : int64 ;
            Memory : boolean ) : longint ;

begin
    if( ( not Memory ) and ( Octal( '777550' ) <= Address ) and ( Octal( '777553' ) >= Address ) ) then
    begin
...
function TPR11.Get_Profiling : boolean ;
...
function TPR11.Get_Signal( Name : PChar ; var State : boolean ) : boolean ;
...
function TPR11.Input_Component( Index : longint ) : TComponent ;
...
const _Name : PChar = 'DEC PR11'#0 ;

function TPR11.Name : PChar ;
...
function TPR11.Output_Component( Index : longint ) : TComponent ;
...
procedure TPR11.Write_Bus( Address : int64 ; Buffer : word ; Size : integer ) ;
...
function TPR11.Read( Address : int64 ; Size : longint ;
...
var Buffer : word ;
    ...
    begin
        if(
            ( Address >= Octal( '777550' ) )
            and
            ( Address <= Octal( '777553' ) )
          ) then
        begin
            Buffer := 0 ;
            Examine( Address, Size, PChar( @Buffer ), True ) ;
            case Address - Octal( '777550' ) of
                0 : Buffer := PRS and $88C0 ;
                1 : Buffer := ( PRS shr 8 ) and $88 ;
            ...
            begin
                ...
                _Logger.Log( self, PChar( cvtb( 10, B, inttostr( Buffer ) ) ), -1, False, LT_Read ) ;
            end ;
...
function TPR11.Restore_Contents( Stream : TCOM_Stream ) : TUnified_Exception ;
...
function TPR11.Restore_State( Stream : TCOM_Stream ) : TUnified_Exception ;
...
function TPR11.Save_Contents( Stream : TCOM_Stream ) : TUnified_Exception ;
...
end ; { TPR11.Save_Contents }


function TPR11.Save_State( Stream : TCOM_Stream ) : TUnified_Exception ;
...
function TPR11.Set_Access_Mode( Low, High : int64 ; Memory : boolean ;
    Typ : longint ) : TUnified_Exception ;

begin
    if( ( not Memory ) and ( Octal( '777550' ) <= Low ) and ( Octal( '777553' ) >= High ) ) then
    begin
        Access_Mode := Typ ;
    end ;
    Set_Access_Mode := Set_Error( CEFPR11Err_Success ) ;
end ;


procedure TPR11.Set_Signal( Name : PChar ; State : boolean ) ;
...
procedure TPR11.Set_Profiling( _On, Children : boolean ) ;
...
function TPR11.Set_Watchpoint( Address : int64 ; Memory : boolean ;
            Access : longint ) : TUnified_Exception ;

begin
    Set_Watchpoint := Set_Error( CEFPR11Err_Success ) ;
    if( ( not Memory ) and ( Octal( '777550' ) <= Address ) and ( Octal( '777553' ) >= Address ) ) then
    begin
...
function TPR11.Signal_Count : longint ;
...
function TPR11.Signal_Name( Index : longint ) : PChar ;
...
function TPR11.Signal_Out( Index : longint ) : boolean ;
...
function TPR11.Signal_Active_Low( Index : longint ) : boolean ;
...
function TPR11.Signal_Index( Name : PChar ) : integer ;
...
function TPR11.Get_Logger : TCEF_Logger ;
...
procedure TPR11.Set_Logger( Value : TCEF_Logger ) ;
...
function TPR11.Write( Address : int64 ; Value, Size : longint ;
    ...
    begin
        ...
        begin
            ...
            case Address - Octal( '777550' ) of
                0 : // PRS
                    begin
                        Value := Value and 65  ; // Clear read-only bits
                    end ;
                1..4 : exit ;
            end ;
...
begin
    ...
    begin
        ...
        begin
            Write := Set_Error( CEFPR11Err_Access_Violation ) ;
            exit ;
        end ;
        Count := ( Size + 7 ) div 8 ;
        while( Count > 0 ) do
    ...
    begin
	    Write := Set_Error( CEFPR11Err_Success ) ;
    end ;
end ;


constructor TPR11_IO.Create ;
...
function TPR11_IO.Set_Address_Range( Low, High : int64 ) : TUnified_Exception ;

begin
    if( ( Low < 0 ) or ( Low > High ) ) then
    begin
	Set_Address_Range := Parent.Set_Error( CEFPR11Err_Invalid_Range ) ;
	exit ;
    end ;
    Set_Address_Range := Parent.Set_Error( CEFPR11Err_Success ) ;
end ;


procedure TPR11_IO.Get_Address_Range( var Low, High : int64 ) ;
...
procedure TPR11.Set_Tag( Value : longint ) ;
...
function TPR11.Get_Tag : longint ;
...
function TPR11.Get_Parent : TComponent ;
...
procedure TPR11.Set_Parent( Component : TComponent ) ;
...
procedure TPR11.Set_Up( P : PChar ) ;
