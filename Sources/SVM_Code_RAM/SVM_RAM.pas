{$N+}
{
        ...
        *************************************************************
        ...
        This is a memory component for CEF which virtualizes access to SVM image
        files.
...
type TImage = class
                  ...
                  Image_File : file ; { Opened image file }
                  Code_Start : longint ; { Starting byte of code in file }
...
type TCEF_Block_Memory = class ;
     ...
     TCEF_Block_Memory = class( TMemory )
        ...
        private { Instance data... }
            ...
            Parent : TCEF_Memory ;
            _Logger : TCEF_Logger ;
            _Files : TList ; // List of mapped files
...
begin
    ...
    case Ordinal of
        ...
        5 : I.Title := PChar( '_Files = ' + Pointer_To_String( Memory._Memory._Files ) ) ;
...
begin
    ...
    Watchpoint_List := nil ;
    for Loop := 0 to _Files.Count - 1 do
    begin
        TImage( _Files[ Loop ] ).Free ;
        _Files[ Loop ] := nil ;
    end ;
    _Files.Free ;
    _Files := nil ;
...
begin
    for Loop := 0 to _Files.Count - 1 do
    begin
        Image := TImage( _Files[ Loop ] ) ;
        closefile( Image.Image_File ) ;
        Image.Free ;
    end ;
    _Files.Clear ;
...
begin
    ...
    if( _Inputs.Indexof( Component ) = -1 ) then
    begin
...
begin
    ...
    if( _Outputs.Indexof( Component ) = -1 ) then
    begin
...
begin
    ...
    begin
	    if( _Inputs.Indexof( Component ) = -1 ) then
        begin
            Result := _Memory.Set_Error( CEFMemoryErr_Component_Not_Found ) ;
            exit ;
        end ;
        _Inputs.Remove( Component ) ;
        if( Component = Default_Input ) then
        begin
            Default_Input := nil ;
        end ;
    end ;
...
begin
    ...
    begin
        ...
        if( Component = Default_Output ) then
        begin
            Default_Output := nil ;
        end ;
    end ;
...
begin
    ...
    begin
        if( Image <> nil ) then
        begin
            if( ( Address < Image.Code_Low ) or ( Address > Image.Code_High ) ) then
            begin
                Image := nil ; // No longer in the last image
            end ;
        end ;
        if( Image = nil ) then
        begin
            // Find image with this address
            for Loop := 0 to _Memory._Files.Count - 1 do
            begin
                Image := TImage( _Memory._Files[ Loop ] ) ;
                if( ( Address >= Image.Code_Low ) and ( Address <= Image.Code_High ) ) then
                begin
                    Deposit := _Memory.Set_Error( CEFMemoryErr_Access_Violation ) ; // Read-only
                    exit ;
                end ;
            end ;
        end ;
        if( Image <> nil ) then // Address mapped to a file
        begin
            if( ( Address >= Image.Code_Low ) and ( Address <= Image.Code_High ) ) then
            begin
                Deposit := _Memory.Set_Error( CEFMemoryErr_Access_Violation ) ; // Read-only
        ...
        // Pass request on to whatever is connected to us
        Deposit := _Memory.Set_Error( CEFMemoryErr_Success ) ;
        if( Default_Input <> nil ) then
        begin
            Result := Default_Input.Deposit( Address, 1, P, Memory ) ;
        end ;
        if( Default_Output <> nil ) then
        begin
            if( Result.Code <> 0 ) then
            begin
                Result := Default_Output.Deposit( Address, 1, P, Memory ) ;
            end else
            begin
                Default_Output.Deposit( Address, 1, P, Memory ) ;
            end ;
        end ;
        dec( Count ) ;
        inc( P ) ;
...
begin
    if( ( not Memory ) or ( _Memory._Low > Address ) or ( _Memory._High < Address ) ) then
    begin
        Result := _Memory.Set_Error( CEFMemoryErr_Address_Out_Of_Range ) ;
        exit ;
    end ;
    if( Size = 0 ) then
    begin
    ...
    begin
        ...
        begin
            if( ( Address < Image.Code_Low ) or ( Address > Image.Code_High ) ) then
            begin
                Image := nil ; // Address is not mapped to a file
            end ;
        end ;

        if( Image = nil ) then // Address not mapped to a file
        begin
            // Pass request on to whatever is connected to us
            if( ( Default_Input = nil ) and ( Default_Output = nil ) ) then
            begin
                Result := _Memory.Set_Error( CEFMemoryErr_Address_Out_Of_Range ) ;
                exit ;
            end ;
            ...
            if( Default_Input <> nil ) then
            begin
                Result := Default_Input.Examine( Address, Size, P, Memory ) ;
            end ;
            if( Default_Output <> nil ) then
            begin
                if( Result.Code <> 0 ) then // Didn't get from input
                begin
                    Result := Default_Output.Examine( Address, Size, P, Memory ) ;
                end else
                begin
                    Default_Output.Examine( Address, Size, P, Memory ) ;
                end ;
            end ;
        end else
        begin
            seek( Image.Image_File, Address - Image.Code_Low + Image.Code_Start ) ;
            blockread( Image.Image_File, P[ 0 ], 1 ) ;
        end ;
...
begin
    ...
    begin
        ...
        begin
            getmem( Buffer, Count ) ;
            S := Size ;
            UEC := Examine( Address, S, Buffer, True ) ;
            if( UEC.Code <> 0 ) then
            begin
                Read := False ;
                exit ;
            end ;
            for Loop := 0 to _Outputs.Count - 1 do
            begin
                TComponent( _Outputs[ Loop ] ).Write_String( Address, Buffer, Size, IO_Type_Memory ) ;
            end ;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                closefile( Image.Image_File ) ;
                Image.Free ;
                _Memory._Files[ Loop ] := nil ;
            end ;
            _Memory._Files.Clear ;
        end else
        if( S = 'LOAD' ) then
        begin
            S := Parser.Token ; // Get filename
            ...
            Image := TImage.Create ;
            Image.Name := S ;
            assignfile( Image.Image_File, S ) ;
            Saved := FileMode ;
            FileMode := fmOpenRead or fmShareDenyNone ;
            {$I-}
            System.reset( Image.Image_File, 1 ) ;
            {$I+}
            Dummy := IOResult ;
            FileMode := Saved ;
            if( Dummy <> 0 ) then
            ...
            // Now process modifiers to the LOAD command...
            while( S <> '' ) do
            begin
                S := uppercase( Parser.Token ) ;
                if( S = '' ) then
                ...
                if( S = 'FILE_OFFSET' ) then // Start of code in file (byte offset)
                begin
                    Image.Code_Start := strtoint( Parser.Token ) ;
                end else
                if( S = 'CODE_OFFSET' ) then // Where to map into RAM (byte address)
                begin
                    Image.Code_Low := strtoint( Parser.Token ) ;
                end else
                if( S = 'SIZE' ) then // Size of code in file (in bytes)
                begin
                    Size := strtoint( Parser.Token ) ;
                end ;
            end ;
            Image.Code_High := Image.Code_Low + Size - 1 ;
            _Memory._Files.Add( Image ) ;
        end ;
    end ; // while( S <> '' )
end ; // TCEF_Memory.Set_Up
